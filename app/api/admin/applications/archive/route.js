  ...
  try {
    ...
    const auditPromises = currentApplications.map(app => {
      ...
      return logAuditEvent({
        ...
        newValues,
        changes: {
          is_archived: { from: oldValues.is_archived, to: newValues.is_archived },
          ...(archive && { archive_reason: { from: null, to: reason || 'manual' } })
        },
    ...
    await logAuditEvent({
      ...
      subcategory: 'ARCHIVE',
      actorId: session?.user?.id,
      actorName: session?.user ? `${session.user.firstName} ${session.user.lastName}`.trim() : 'Unknown',
