...
export async function logAuditEvent(eventConfig, request = null) {
  try {
    ...
    const auditData = {
      ...
      entityType: eventConfig.entityType || null,
      entityId: eventConfig.entityId || null,
      entityName: eventConfig.entityName || null,
      actorId: session?.user?.id || eventConfig.actorId || null,
      actorType: session?.user ? 'user' : eventConfig.actorType || 'system',
      actorName: session?.user ? `${session.user.firstName || ''} ${session.user.lastName || ''}`.trim() || session.user.email : eventConfig.actorName || 'System',
      action: eventConfig.action,
      description: eventConfig.description || null,
      oldValues: eventConfig.oldValues || null,
      newValues: eventConfig.newValues || null,
      changes: eventConfig.changes || null,
      ipAddress: requestContext.ipAddress || eventConfig.ipAddress || null,
      userAgent: requestContext.userAgent || eventConfig.userAgent || null,
      sessionId: session?.user?.id || eventConfig.sessionId || null,
      requestId: requestContext.requestId || eventConfig.requestId || null,
      relatedUserId: eventConfig.relatedUserId || null,
      relatedJobId: eventConfig.relatedJobId || null,
      relatedApplicationId: eventConfig.relatedApplicationId || null,
