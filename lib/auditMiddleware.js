...
export function withAuditLogging(handler, auditConfig) {
  return async function auditedHandler(request, context) {
    ...
    try {
      // Get session and request context - import dynamically to avoid build-time execution
...
export async function logAuditEvent(eventConfig, request = null) {
  try {
    ...
    const auditData = {
      event_type: truncateForDb(eventConfig.eventType, 100),
      category: truncateForDb(eventConfig.category, 50),
      subcategory: truncateForDb(eventConfig.subcategory, 50) || null,
      entity_type: truncateForDb(eventConfig.entityType, 50) || null,
      entity_id: eventConfig.entityId || null,
      entity_name: truncateForDb(eventConfig.entityName, 255) || null,
      actor_id: session?.user?.id || eventConfig.actorId || null,
      actor_type: truncateForDb(session?.user ? 'user' : eventConfig.actorType || 'system', 20),
      actor_name: truncateForDb(session?.user ? `${session.user.firstName || ''} ${session.user.lastName || ''}`.trim() || session.user.email : eventConfig.actorName || 'System', 255),
      action: truncateForDb(eventConfig.action, 255),
      ...
      ip_address: truncateForDb(requestContext.ipAddress || eventConfig.ipAddress, 45) || null,
      user_agent: truncateForDb(requestContext.userAgent || eventConfig.userAgent, 500) || null,
      session_id: truncateForDb(session?.user?.id || eventConfig.sessionId, 255) || null,
      request_id: truncateForDb(requestContext.requestId || eventConfig.requestId, 255) || null,
      ...
      severity: truncateForDb(eventConfig.severity || 'info', 20),
      status: truncateForDb(eventConfig.status || 'success', 20),
...
/**
 * Helper function to safely truncate strings for varchar fields
 */
export function truncateForDb(str, maxLength = 255) {
  if (!str) return str;
  return typeof str === 'string' && str.length > maxLength ? str.substring(0, maxLength - 3) + '...' : str;
}
